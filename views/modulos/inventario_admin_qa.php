...
if ($_GET['idt'] && $_GET['name']) {
    ...
    if ($consultaToken) {
        ...
        if ($idtad['token'] != $idt) {
            ...
            $doc_generales = ControladorFormularioCalidad::ctrSeleccionarRegistros('inventario');
        }
    } else {
        echo '<script>window.location ="' . $dirIndexOnline . '"</script>';
        return;
...
<form action="#" class="bg-light" method="post" enctype="multipart/form-data">
    <div class="container-fluid justify-content-center align-items-center d-flex flex-column py-4">
        ...
        <?php ControladorFormularioCalidad::ctrIngresoDocumentosGeneral($idt, $name, 'inventario','../views/inventario/','inventario_admin_qa') ?>
    </div>
...
<?php ControladorFormularioCalidad::ctrDownloadDocs('inventario','http://localhost/cpi_login_calidad/views/inventario/');
ControladorFormularioCalidad::ctrEliminarDocumentoGeneral('inventario','../views/inventario/');
