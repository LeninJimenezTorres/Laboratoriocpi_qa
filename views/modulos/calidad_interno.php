...
if ($_GET['idt'] && $_GET['name']) {
    ...
    if ($consultaToken) {
        ...
        if ($idtad['token'] != $idt) {
            ...
            $doc_generales = ControladorFormularioCalidad::ctrSeleccionarRegistros('calidad_interno');
        }
    } else {
        echo '<script>window.location ="' . $dirIndexOnline . '"</script>';
        return;
...
<form action="#" class="bg-light" method="post" enctype="multipart/form-data">
    <div class="container-fluid justify-content-center align-items-center d-flex flex-column py-4">
        ...
        <?php ControladorFormularioCalidad::ctrIngresoDocumentosCalidad($idt, $name, 'calidad_interno', 'interno') ?>
    </div>
    <div class="container-fluid p-2 lola bg-light">
        <!--bg-light-->
        <h4 class="h3-title text-center"><br><br>Lista de documentos de acceso a personal interno</h4>
...
<?php ControladorFormularioCalidad::ctrDownloadCalidad('interno');
ControladorFormularioCalidad::ctrEliminarDocumentoCalidad('interno');
