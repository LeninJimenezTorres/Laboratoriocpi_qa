...
if ($_GET['idt'] && $_GET['name']) {
    ...
    $dirPanelUser = '../views/panel_usuario_calidad.php';
    $dirPanelUserModuloSalir = '../views/panel_usuario_calidad.php?modulos=salir_admin_qa';
    ...
    if ($consultaToken) {
        ...
        if ($idtad['token'] != $idt) {
            ...
            $doc_generales = ControladorFormularioCalidad::ctrSeleccionarRegistros('calidad_general');
        }
    } else {
        echo '<script>window.location ="' . $dirIndexOnline . '"</script>';
        return;
...
<form action="#" class="bg-light" method="post" enctype="multipart/form-data">
    <div class="container-fluid justify-content-center align-items-center d-flex flex-column py-4">
        ...
        <?php ControladorFormularioCalidad::ctrIngresoDocumentosCalidad($idt, $name, 'calidad_general', 'general') ?>
    </div>
    <div class="container-fluid p-2 lola bg-light">
        <!--bg-light-->
        <h4 class="h3-title text-center"><br><br>Lista de documentos de acceso general</h4>
                        ...
                        <td>
                            <div class="row justify-content-center align-items-center form-group">
                                <form action="" method="post">
                                    <button type="submit" class="btn btn-danger" name="eliminar" value="<?php echo $value["name"] ?>"><i class="fas fa-trash-alt"></i></button>
                                </form>
                            </div>
                        </td>
                    </tr>
...
<?php ControladorFormularioCalidad::ctrDownloadCalidad('general');
ControladorFormularioCalidad::ctrEliminarDocumentoCalidad('general');
